...
def find_list(path_to_TIMIT):
    ...
    for part in train_dir.iterdir():
        if (part.is_file()):
            continue
        phn_list = part.iterdir()
        phn_list = [x for x in phn_list if x.name.endswith('phn')]
...
def convert_to_pkl(path_list):
    ...
    print(
        f"VoN Records: {len(pickle_dict['VoN'])}, VoT Records: {len(pickle_dict['VoT'])}, VnN Records: {len(pickle_dict['VnN'])}, VnT Records: {len(pickle_dict['VnT'])}"
    )
