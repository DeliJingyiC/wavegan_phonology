...
##manipulate latent variables to [-5,5]
def get_manipulate_z(
    wavegan_cate: int,
    batch_size,
    latent_dim,
    alter_axis: List[int],
    alter_vals: np.ndarray,
    with_grad: bool = True,
    use_cuda: bool = True,
    random_range=1,
    initialize_mode: str = "uniform",
    control_non_alter_vals: bool = True,
    fix_laten_vals_accross_alter_vals: bool = True,
):
    import itertools
    #
    alter_vals_axis = list(
        itertools.product(alter_vals, repeat=len(alter_axis)))
    alter_vals_axis = np.array(alter_vals_axis)
    if (fix_laten_vals_accross_alter_vals):
        if (initialize_mode == "uniform"):
            noise = np.random.uniform(
                -random_range,
                random_range,
                size=(batch_size, latent_dim),
            )
        elif (initialize_mode == "normal"):
            noise = np.random.normal(
                loc=0,
                scale=1,
                size=(batch_size, latent_dim),
            )
        if (control_non_alter_vals):
            for i in range(len(noise)):
                noise[i, :] = noise[0, :]
        sec = noise[:, :wavegan_cate]
        sec[sec > 0] = 1
        sec[sec <= 0] = 0
    print(len(alter_vals_axis))
    assert len(alter_vals_axis) == len(alter_vals)**len(
        alter_axis
    ), f"{len(alter_vals_axis)}=={len(alter_vals)**len(alter_axis)}"
    # total_batches = int(np.ceil(len(alter_vals_axis) / batch_size))
    total_batches = len(alter_vals_axis)
    for i in range(total_batches):
        if (fix_laten_vals_accross_alter_vals):
            noise_sub = np.copy(noise)
        else:
            ...
            sec[sec <= 0] = 0
            noise_sub = noise
        noise_sub[:, alter_axis] = alter_vals_axis[i]
        noise_sub = torch.tensor(noise_sub, dtype=torch.float)
        if use_cuda:
            noise_sub = noise_sub.cuda()
        if (with_grad):
            noise_v = autograd.Variable(noise_sub)
        else:
            with torch.no_grad():
                noise_v = autograd.Variable(noise_sub)
        yield noise_v, noise_sub[:, alter_axis].cpu(), i, total_batches
