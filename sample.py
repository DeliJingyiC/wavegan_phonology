...
class AudioDataset(torch.utils.data.Dataset):
    def __init__(self, audio: np.ndarray):
        ...
        self.audioTensor = torch.from_numpy(audio).unsqueeze(dim=1).float()
...
def file_sample_generator(filepath, window_length=16384, fs=16000):
    ...
    """
    try:
        audio_data, _ = sf.read(filepath, sr=fs)
...
def pickle_sample_generator(data, window_length=16384):
    """
    Audio sample generator
    """

    audio_data, _ = data["Sound"], data["Sample Rate"]

    audio_data = signal.resample(audio_data,
                                 int(len(audio_data) / (_ / 16000)))
    # Clip amplitude
    max_amp = np.max(np.abs(audio_data))
    if max_amp > 1:
        audio_data /= max_amp
    ...
    if audio_len < window_length:
        ...
        raise Exception(
            f"pickle_sample_generator: audio is longer than {window_length}")
...
def create_dataset_pickle(data_pickle, valid_ratio, test_ratio,
                          train_batch_size, valid_size, test_size,
                          window_size):
    num_files = len(data_pickle)
    num_valid = int(np.ceil(num_files * valid_ratio))
    num_test = int(np.ceil(num_files * test_ratio))
    num_train = num_files - num_valid - num_test
    ...
    valid_files = data_pickle[:num_valid]
    test_files = data_pickle[num_valid:num_valid + num_test]
    train_files = data_pickle[num_valid + num_test:]

    train_data = np.array(
        [pickle_sample_generator(x, window_size)["X"] for x in train_files])
    valid_data = np.array(
        [pickle_sample_generator(x, window_size)["X"] for x in valid_files])
    test_data = np.array(
        [pickle_sample_generator(x, window_size)["X"] for x in test_files])
